...
class SonarCallback {
public:
	virtual void OnPing(AsyncSonar&) = 0;
	virtual void OnTimeOut(AsyncSonar&) = 0;
};
...
public:
	...
	uint8_t TriggerPin;
	SonarCallback *callback;
	//SonarCallback OnPing = nullptr;
	//SonarCallback OnTimeOut = nullptr;

#ifndef ASYNCSONAR_USE_SONARISR
	AsyncSonar(uint8_t trigger_pin, SonarCallback &callback)
		: TriggerPin(trigger_pin), callback(&callback) {}
